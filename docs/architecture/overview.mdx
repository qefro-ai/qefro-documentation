...
> **Configure** knowledge, tools, and Marketplace apps in the Admin Console; **execute** chat and flows in a single Runtime (RAG + FlowRunner + event bus); **reach customers and employees** through channels; **run Marketplace Apps as metadata** on Qefro Runtime. Connect **external** ERP / POS / CRM systems with the Backend SDK (`/qefro`) or REST/OpenAPI. Connectors only **emit events** — they never run a second orchestrator.
