...
description: "Use @qefro/backend or qefro-sdk to register handlers, connect your webhook in Admin Console, and sync tools into a workspace as Business Tools."
...
This guide registers **Business Tools** from your backend using the Qefro Backend Framework (`@qefro/backend` or `qefro-sdk`). Tools live in your code; Admin Console only manages the signed connection and syncs metadata into a workspace.
...
- Node.js 18+ (TypeScript) **or** Rust for `qefro-sdk`
...
sequenceDiagram
  ...
  participant SDK as Your @qefro/backend app
...
<WorkflowCard
  ...
  steps={[
    {title: 'Install framework', description: '@qefro/backend or qefro-sdk + signing secret.'},
...
<FaqAccordion
  items={[
    ...
    },
    {
      ...
      answer:
        'Yes. See docs/examples (basic-sdk and industry samples) and run scripts/smoke.sh for signed ping / tools.list / tool.invoke checks.',
    },
  ]}
/>
