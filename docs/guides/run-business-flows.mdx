...
When `metadata.trigger` is `event`, `schedule`, or `webhook`, the flow does **not** start from chat intent. A producer emits a namespaced event into the org event bus; the dispatcher matches enabled flows and starts a system `Channel::Event` conversation + run. Payload fields become seed variables. Full guide: [Event-Driven Triggers](/docs/guides/event-driven-triggers).
...
<FaqAccordion
  items={[
    ...
    {
      question: 'Can the same flow start from both chat and a connector event?',
      answer:
