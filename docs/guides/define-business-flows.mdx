...
Flows are **metadata only**. The SDK advertises them through the `capabilities.list` message; the Qefro Runtime discovers, validates, versions, and (in a later phase) executes them. **Nothing runs inside the SDK.** Register your tools first with [Register SDK Business Tools](/docs/guides/register-sdk-business-tools) — a flow references those tools by name.

<InfoBox>
Flows never run in your backend and never change how individual tools behave. They are a declarative description the Runtime reads. This phase stores and displays flows; execution ships later.
</InfoBox>
...
| `delay` | Wait before continuing | `duration_seconds` |
  ...
  items={[
    {
      ...
      answer:
        'No. Flows are metadata only. The SDK advertises them through capabilities.list; the Qefro Runtime stores and displays them, and executes them in a later phase. Your tools behave exactly as before.',
    },
    {
