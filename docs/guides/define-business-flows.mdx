...
description: "Advertise Business Flows from your backend with @qefro-ai/backend, qefro-backend-sdk, or qefro-backend. The Qefro Runtime discovers, validates, and versions flows via capabilities.list — the SDK never executes them."
...
Flows are **metadata only** in your backend. The SDK advertises them through the `capabilities.list` message; the Qefro Runtime discovers, validates, versions, and **executes** them. **Nothing runs inside the SDK.** Register your tools first with [Register SDK Business Tools](/docs/guides/register-sdk-business-tools) — a flow references those tools by name. Once a flow is enabled, see [Run Business Flows](/docs/guides/run-business-flows) for conversation-driven execution, or [Event-Driven Triggers](/docs/guides/event-driven-triggers) to start the **same** Runtime from connectors, webhooks, and schedules.
...
| `trigger` | Optional entry path: `conversation` (default), `event`, `schedule`, or `webhook` — see [Event-Driven Triggers](/docs/guides/event-driven-triggers) |
...
Event names must be **namespaced** (`shopify.cart.abandoned`, not `cart_abandoned`). Full emit, ordering, replay, and connector guidance: [Event-Driven Triggers](/docs/guides/event-driven-triggers).
